...
import OperatorAdminSignIn from "./components/OperatorAdminSignIn";
...
export default function OperatorAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (loading) return;
    if (user && profile && profile.role !== "operatorAdmin") {
      setWrongRole(true);
      firebaseSignOut();
    }
  ...
  if (!user || !profile || profile.role !== "operatorAdmin") {
    return <OperatorAdminSignIn wrongRoleError={wrongRole} />;
  }
