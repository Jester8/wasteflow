...
export default function AddDriverModal({ onClose }: { onClose: () => void }) {
  const [fullName, setFullName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [mobileNumber, setMobileNumber] = useState("");
  const [vehicleRegistration, setVehicleRegistration] = useState("");
  const [skipCapacity, setSkipCapacity] = useState<number[]>([]);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");

  function toggleSize(size: number) {
    setSkipCapacity((prev) => prev.includes(size) ? prev.filter((s) => s !== size) : [...prev, size]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/operator-admin/create-driver", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          idToken, fullName: fullName.trim(), email: email.trim(), password,
          mobileNumber: mobileNumber.trim(), vehicleRegistration: vehicleRegistration.trim().toUpperCase(),
          skipCapacity,
        }),
      });
      const result = await res.json();
      if (!res.ok) {
        setError(result.error || "Failed to create fleet operator.");
        return;
      }
      onClose();
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontSize: "1rem", fontWeight: 800, color: "#1a2e1f", margin: 0 }}>Add Fleet Operator</p>
        ...
        {[
          { label: "Driver name", value: fullName, onChange: setFullName, placeholder: "e.g. John Adeyemi" },
          { label: "Email", value: email, onChange: setEmail, placeholder: "driver@company.co.uk", type: "email" },
          ...
          { label: "Vehicle registration", value: vehicleRegistration, onChange: setVehicleRegistration, placeholder: "e.g. LX21 KLM" },
        ].map((f) => (
          <div key={f.label}>
        ...
        <div>
          <label style={{ fontSize: "0.78rem", fontWeight: 600, color: "#2a5c38", display: "block", marginBottom: 6 }}>
            Skip sizes this vehicle can carry
          </label>
            ...
            {SKIP_SIZES.map((size) => (
              <button
                ...
                style={{
                  ...
                  border: skipCapacity.includes(size) ? "1.5px solid #1a4d2e" : "1px solid #e8f2eb",
                  background: skipCapacity.includes(size) ? "#1a4d2e" : "#fff",
                  color: skipCapacity.includes(size) ? "#B8D52E" : "#4a7a5a",
