...
export default function OperatorRequestDetailModal({ item, onClose, onResubmit, onAcceptPickup, onReschedulePickup }) {
  ...
  const assignedOperatorId = item?.assignedOperatorId || item?.operatorId || "";

  useEffect(() => {
    setDriverContact(null);
    if (!assignedOperatorId) return;
    getDoc(doc(db, "users", assignedOperatorId)).then((snap) => {
      if (!snap.exists()) return;
      const data = snap.data();
      setDriverContact({ fullName: data.fullName || "Driver", mobileNumber: data.mobileNumber || "" });
    });
  }, [assignedOperatorId]);
              ...
              <div className="or-grid">
                ...
                {driverContact.mobileNumber && (
                  <DetailRow
                    ...
                    value={driverContact.mobileNumber}
                    last
                  />
                )}
              </div>
