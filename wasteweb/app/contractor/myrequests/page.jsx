...
const TRACKABLE_STATUSES = new Set(["Arriving", "In Transit"]);
...
// ── NEW: Live tracking banner shown inline in the drawer body ─────────────────
function LiveTrackingBanner({ requestId, onTrack }) {
  return (
    <div
      onClick={onTrack}
      role="button"
    ...
    >
      ...
      <div style={{ position: "relative", width: 36, height: 36, flexShrink: 0 }}>
        <span style={{
          ...
          background: "rgba(184,213,46,0.3)",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{
          fontSize: "0.82rem", fontWeight: 700, color: "#B8D52E",
          fontFamily: "'Quicksand', sans-serif", margin: 0,
        }}>
          Operator is on the way — Track Live
        </p>
        <p style={{
          fontSize: "0.71rem", fontWeight: 600, color: "rgba(184,213,46,0.65)",
          fontFamily: "'Quicksand', sans-serif", margin: "2px 0 0",
        }}>
          See real-time location and ETA on the map
        </p>
      </div>

      <svg viewBox="0 0 24 24" fill="none" stroke="#B8D52E" strokeWidth="2.2"
...
function AcceptanceConfirmModal({ item, onClose, onAccept, onReschedule }) {
  ...
  async function handleAccept() {
    setSubmitting(true);
    try { await onAccept?.(item); setView("success"); }
    finally { setSubmitting(false); }
  }

  async function handleRescheduleSend() {
    setSubmitting(true);
    try { await onReschedule?.(item, reschedule); setView("reschedule-sent"); }
    finally { setSubmitting(false); }
  }
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes wfPulse      { 0%{box-shadow:0 0 0 0 rgba(184,213,46,0.5)} 70%{box-shadow:0 0 0 10px rgba(184,213,46,0)} 100%{box-shadow:0 0 0 0 rgba(184,213,46,0)} }

        .ac-backdrop {
          position:fixed; inset:0; z-index:500;
          background:rgba(10,22,13,0.55); backdrop-filter:blur(4px);
        ...
        .ac-header {
          padding:18px 20px 14px; border-bottom:1px solid #f0f7f2;
        ...
        .ac-close {
          width:30px; height:30px; border-radius:8px; background:#f5faf6; border:1px solid #e8f2eb;
          display:flex; align-items:center; justify-content:center; cursor:pointer; color:#4a7a5a; flex-shrink:0;
        ...
        .ac-btn-primary {
          width:100%; padding:11px 16px; border-radius:10px; background:#1a4d2e; border:none; cursor:pointer;
          color:#B8D52E; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
          display:flex; align-items:center; justify-content:center; gap:7px; transition:background 0.15s, color 0.15s;
        }
        .ac-btn-primary:disabled { opacity:0.65; cursor:not-allowed; }
        .ac-btn-primary:hover:not(:disabled) { background:#B8D52E; color:#0d2416; }
        .ac-btn-secondary {
          width:100%; padding:11px 16px; border-radius:10px; background:none; border:1px solid #e8f2eb; cursor:pointer;
          color:#1a4d2e; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
          display:flex; align-items:center; justify-content:center; gap:7px; transition:background 0.15s, border-color 0.15s;
        }
        .ac-btn-secondary:hover { background:#f0f7f2; border-color:#B8D52E; }
        .ac-field-label {
          font-size:0.68rem; font-weight:700; color:#9ab8a5; text-transform:uppercase; letter-spacing:0.06em;
          margin-bottom:6px; font-family:'Quicksand',sans-serif; display:block;
        }
        .ac-input {
          width:100%; padding:9px 11px; border-radius:8px; border:1.5px solid #e8f2eb; background:#fff;
          font-size:0.82rem; font-weight:600; color:#1a2e1f; font-family:'Quicksand',sans-serif; outline:none;
          transition:border 0.15s, box-shadow 0.15s; box-sizing:border-box; colorScheme:light;
        }
        .ac-input:focus { border-color:#B8D52E; box-shadow:0 0 0 3px rgba(184,213,46,0.12); }
        .ac-textarea {
          width:100%; padding:9px 11px; border-radius:8px; border:1.5px solid #e8f2eb; background:#fff;
          font-size:0.82rem; font-weight:600; color:#1a2e1f; font-family:'Quicksand',sans-serif;
          outline:none; resize:none; min-height:72px; transition:border 0.15s, box-shadow 0.15s; box-sizing:border-box;
        }
        ...
        .ac-back-btn {
          background:none; border:none; cursor:pointer; font-size:0.78rem; font-weight:700; color:#6b8f7a;
          font-family:'Quicksand',sans-serif; display:flex; align-items:center; gap:5px; padding:0;
        }
        .ac-back-btn:hover { color:#1a4d2e; }
        .ac-info-card { background:#f5faf6; border:1px solid #e8f2eb; border-radius:10px; padding:12px 14px; display:flex; flex-direction:column; gap:8px; }
        .ac-info-divider { height:1px; background:#e8f2eb; margin:2px 0; }
        .ac-info-label { font-size:0.68rem; font-weight:700; color:#9ab8a5; text-transform:uppercase; letter-spacing:0.05em; font-family:'Quicksand',sans-serif; margin:0 0 2px; }
        .ac-info-val { font-size:0.85rem; font-weight:700; color:#1a2e1f; font-family:'Quicksand',sans-serif; margin:0; }
        .ac-driver-card { display:flex; align-items:center; gap:10px; background:rgba(184,213,46,0.08); border:1px solid rgba(184,213,46,0.25); border-radius:10px; padding:10px 13px; }
        .ac-driver-avatar { width:36px; height:36px; border-radius:50%; background:#1a4d2e; color:#B8D52E; display:flex; align-items:center; justify-content:center; font-size:0.72rem; font-weight:800; flex-shrink:0; font-family:'Quicksand',sans-serif; }
      ...
      <div
        ...
        role="dialog" aria-modal="true" aria-label="Pickup acceptance"
      >
        <div className="ac-modal">
            ...
            <>
              <div className="ac-header">
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  ...
                  <div>
                    <p style={{ fontSize: "0.95rem", fontWeight: 800, color: "#1a2e1f", fontFamily: "'Quicksand', sans-serif", lineHeight: 1.3, margin: 0 }}>
                      Your request has been accepted by an operator
                    </p>
                    <p style={{ fontSize: "0.73rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", marginTop: 3 }}>
                      Review the proposed schedule and confirm or reschedule
                    </p>
                  </div>
                </div>
                <button className="ac-close" onClick={onClose} disabled={submitting}><XIcon size={13} /></button>
              </div>
              ...
              <div className="ac-footer">
                <button className="ac-btn-primary" onClick={handleAccept} disabled={submitting}>
                  {submitting ? <><div className="ac-spinner" /> Confirming…</> : <><CheckIcon /> Accept pickup</>}
                </button>
            ...
            <>
              <div className="ac-header">
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  ...
                  <div>
                    <p style={{ fontSize: "0.95rem", fontWeight: 800, color: "#1a2e1f", fontFamily: "'Quicksand', sans-serif", lineHeight: 1.3, margin: 0 }}>Request a reschedule</p>
                    <p style={{ fontSize: "0.73rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", marginTop: 3 }}>The operator will choose from your suggested window</p>
                  </div>
                </div>
                <button className="ac-close" onClick={() => setView("confirm")} disabled={submitting}><XIcon size={13} /></button>
              </div>
              ...
              <div className="ac-body">
                ...
                <div>
                  ...
                  <textarea className="ac-textarea" placeholder="e.g. Gate is locked before 9am, please call ahead..."
                    value={reschedule.note} onChange={(e) => setReschedule((p) => ({ ...p, note: e.target.value }))} />
                </div>
              </div>

              <div className="ac-footer">
                <button className="ac-btn-primary" onClick={handleRescheduleSend} disabled={submitting || !canSendReschedule}>
                  {submitting
          ...
          {view === "success" && (
            <div style={{ padding: "36px 24px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              ...
              <p style={{ fontSize: "1rem", fontWeight: 800, color: "#1a2e1f", fontFamily: "'Quicksand', sans-serif", margin: 0 }}>Pickup confirmed!</p>
              <p style={{ fontSize: "0.78rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", lineHeight: 1.55, maxWidth: 290, margin: 0 }}>
              ...
              <button className="ac-btn-primary" style={{ marginTop: 6 }} onClick={onClose}>Done</button>
            </div>
          )}
          ...
          {view === "reschedule-sent" && (
            <div style={{ padding: "36px 24px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              ...
              <p style={{ fontSize: "1rem", fontWeight: 800, color: "#1a2e1f", fontFamily: "'Quicksand', sans-serif", margin: 0 }}>Reschedule request sent</p>
              <p style={{ fontSize: "0.78rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", lineHeight: 1.55, maxWidth: 290, margin: 0 }}>
                The operator will review your suggested window and confirm a new time. You'll be notified once they respond.
              </p>
              <button className="ac-btn-secondary" style={{ marginTop: 6 }} onClick={onClose}>Close</button>
            </div>
          )}
...
export default function OperatorRequestDetailModal({ item, onClose, onResubmit, onAcceptPickup, onReschedulePickup }) {
  ...
  const isTrackable = item ? TRACKABLE_STATUSES.has(item.status) : false;

  const handleTrack = () => {
    onClose?.();
    router.push(`/contractor/tracking/${item.id}`);
  };
  ...
  useEffect(() => {
    if (!item) return;
    if (item.status === "Scheduled") setShowAcceptance(true);
  }, [item?.status]);
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes wfPulse      { 0%{opacity:0.6;transform:scale(1)} 70%{opacity:0;transform:scale(2.2)} 100%{opacity:0;transform:scale(2.2)} }
        ...
        .or-drawer {
          width:100%; max-width:460px; height:100%; background:#ffffff;
        ...
        .or-header {
          ...
          padding:20px 24px; border-bottom:1px solid #f0f7f2; flex-shrink:0; background:#ffffff;
        }
        .or-close {
          width:34px; height:34px; border-radius:9px; background:#f5faf6; border:1px solid #e8f2eb;
          display:flex; align-items:center; justify-content:center; cursor:pointer; color:#4a7a5a;
        ...
        .or-body { flex:1; overflow-y:auto; padding:24px; display:flex; flex-direction:column; }
        ...
        .or-footer {
          flex-shrink:0; padding:16px 24px; border-top:1px solid #f0f7f2;
          background:#ffffff; display:flex; flex-direction:column; gap:8px;
        }
        .or-btn-primary {
          width:100%; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:12px 20px; border-radius:10px; background:#1a4d2e; border:none; cursor:pointer;
          color:#B8D52E; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
        ...
        .or-btn-secondary {
          width:100%; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:12px 20px; border-radius:10px; background:none; border:1px solid #e8f2eb; cursor:pointer;
          color:#1a4d2e; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
        ...
        .or-btn-track:hover { filter:brightness(1.1); }

        .or-btn-accent {
          width:100%; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:12px 20px; border-radius:10px; background:rgba(59,130,246,0.08);
          border:1px solid rgba(59,130,246,0.25); cursor:pointer;
          color:#1d4ed8; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
          transition:background 0.18s, border-color 0.18s;
        }
        .or-btn-accent:hover { background:rgba(59,130,246,0.15); border-color:rgba(59,130,246,0.45); }
        .or-btn-danger {
          width:100%; display:flex; align-items:center; justify-content:center; gap:8px;
          padding:12px 20px; border-radius:10px; background:none; border:1px solid #fca5a5; cursor:pointer;
          color:#b91c1c; font-size:0.88rem; font-weight:700; font-family:'Quicksand',sans-serif;
          transition:background 0.18s, border-color 0.18s;
        }
        .or-btn-danger:hover { background:#fee2e2; border-color:#ef4444; }
        .or-pending-nudge {
          display:flex; align-items:center; gap:10px; background:rgba(59,130,246,0.06);
          border:1px solid rgba(59,130,246,0.22); border-radius:10px; padding:11px 14px;
          cursor:pointer; transition:background 0.15s;
        }
        .or-pending-nudge:hover { background:rgba(59,130,246,0.11); }
        @media (max-width:480px) { .or-drawer { max-width:100%; } }
      `}</style>
        ...
        <div className="or-drawer">
          ...
          <div className="or-header">
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#9ab8a5", fontFamily: "'Quicksand', sans-serif", letterSpacing: "0.06em", textTransform: "uppercase" }}>
                Request · {item.id}
              </span>
            ...
            <button className="or-close" onClick={onClose} aria-label="Close"><XIcon /></button>
          </div>
          ...
          <div className="or-body">
            ...
            {/* ── Live tracking banner (Arriving / In Transit only) ── */}
            {isTrackable && (
              <LiveTrackingBanner requestId={item.id} onTrack={handleTrack} />
            )}
            ...
            {isScheduled && item.awaitingUserConfirmation && (
              ...
              >
                <div style={{ width: 34, height: 34, borderRadius: 9, background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.25)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: "0.78rem", fontWeight: 700, color: "#1d4ed8", fontFamily: "'Quicksand', sans-serif", margin: 0 }}>Action required — confirm your pickup</p>
                  <p style={{ fontSize: "0.7rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", margin: "2px 0 0" }}>Tap to accept or request a new time</p>
                </div>
            ...
            <div style={{ marginTop: 8 }}>
              <DetailRow label="Location" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>} value={item.location} />
              <DetailRow label="Pickup Period" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} value={item.dates} />
              {item.availability && <DetailRow label="Access Time" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>} value={item.availability} />}
              <DetailRow label="Waste Type" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>} value={item.wasteType} />
              <DetailRow label="Total Volume" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>} value={item.volume} valueColor="#1a4d2e" />
              {item.note && <DetailRow label="Notes" icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>} value={item.note} valueColor="#6b8f7a" last />}
            </div>

            {imagesArray.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <p style={{ fontSize: "0.68rem", fontWeight: 700, color: "#9ab8a5", fontFamily: "'Quicksand', sans-serif", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Site Documentation Images</p>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 10 }}>
                  {imagesArray.map((imgSrc, index) => (
                    <div key={index} style={{ aspectRatio: "1", borderRadius: 8, overflow: "hidden", background: "#f5faf6", border: "1px solid #e8f2eb" }}>
                      <img src={imgSrc} alt={`Waste preview ${index + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.currentTarget.style.display = "none"; }} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {isCompleted && (
              <div style={{ marginTop: 24, padding: "16px", background: "#f5faf6", borderRadius: 12, border: "1px solid #e8f2eb" }}>
                <p style={{ fontSize: "0.68rem", fontWeight: 700, color: "#4a7a5a", fontFamily: "'Quicksand', sans-serif", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 10px 0" }}>Completion Details</p>
                {item.operatorName && (
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: "0.72rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", margin: "0 0 2px 0" }}>Operator Name</p>
                    <p style={{ fontSize: "0.88rem", fontWeight: 700, color: "#1a2e1f", fontFamily: "'Quicksand', sans-serif", margin: 0 }}>{item.operatorName}</p>
                  </div>
                )}
                {item.proofPhotoUrl && (
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: "0.72rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", margin: "0 0 6px 0" }}>Site Photo</p>
                    <div style={{ borderRadius: 8, overflow: "hidden", background: "#ffffff", border: "1px solid #e8f2eb" }}>
                      <img src={item.proofPhotoUrl} alt="Site completion photo" style={{ width: "100%", maxHeight: 200, objectFit: "cover" }} onError={(e) => { e.currentTarget.style.display = "none"; }} />
                    </div>
                  </div>
                )}
                {item.signatureUrl && (
                  <div>
                    <p style={{ fontSize: "0.72rem", fontWeight: 600, color: "#6b8f7a", fontFamily: "'Quicksand', sans-serif", margin: "0 0 6px 0" }}>Operator Signature</p>
                    <div style={{ background: "#ffffff", borderRadius: 8, padding: 8, display: "flex", justifyContent: "center", alignItems: "center", border: "1px dashed #c6e2d0", height: 80 }}>
                      <img src={item.signatureUrl} alt="Digital Signature" style={{ maxHeight: "100%", maxWidth: "100%", objectFit: "contain" }} />
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="or-footer">
            ...
            {/* ── Track Live button (Arriving / In Transit) ── */}
            {isTrackable && (
              <button className="or-btn-track" onClick={handleTrack}>
                <span style={{ position: "relative", width: 10, height: 10, flexShrink: 0 }}>
                  <span style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "#B8D52E", animation: "wfPulse 1.8s ease-out infinite" }} />
                  <span style={{ position: "absolute", inset: 2, borderRadius: "50%", background: "#B8D52E" }} />
                </span>
                Track Live
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"
                  strokeLinecap="round" strokeLinejoin="round" style={{ width: 14, height: 14 }}>
            ...
            {isDeclined && (
              <>
                <button className="or-btn-primary" onClick={() => { onClose?.(); onResubmit?.(item); }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"
                    strokeLinecap="round" strokeLinejoin="round" style={{ width: 15, height: 15 }}>
            ...
            {!isCompleted && !isDeclined && !isAwaitingApproval && !isScheduled && !isTrackable && (
              <button className="or-btn-secondary" onClick={onClose}>Close</button>
            )}
      ...
      {showAcceptance && (
        <AcceptanceConfirmModal
          ...
          onAccept={async (req) => { await onAcceptPickup?.(req); }}
          onReschedule={async (req, rescheduleData) => { await onReschedulePickup?.(req, rescheduleData); }}
        />
      )}
    </>
  );
