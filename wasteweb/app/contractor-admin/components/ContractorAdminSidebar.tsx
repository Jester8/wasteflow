...
const NAV_ITEMS = [
  { label: "Overview", href: "/contractor-admin", exact: true, icon: (
  ...
  { label: "My Contractors", href: "/contractor-admin/contractors", icon: (
  ...
  { label: "Requests", href: "/contractor-admin/requests", icon: (
...
interface ContractorAdminSidebarProps {
  collapsed: boolean;
  onCollapse: () => void;
  onExpand: () => void;
  isOpen: boolean;
...
export default function ContractorAdminSidebar({
  collapsed,
  onCollapse,
  onExpand,
  isOpen,
  onClose,
}: ContractorAdminSidebarProps) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        {!collapsed ? (
          ...
          <p style={{ fontSize: "0.9rem", fontWeight: 800, color: "#B8D52E", margin: 0 }}>CA</p>
        )}

        <button
          onClick={collapsed ? onExpand : onCollapse}
          className="ca-icon-btn ca-desktop-toggle"
        ...
        <button onClick={onClose} className="ca-icon-btn ca-mobile-close" aria-label="Close menu">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className={`ca-nav-link${active ? " ca-nav-active" : ""}`}
              style={{
  ...
  return (
    <>
      <style>{`
        ...
        .ca-icon-btn:hover { color: #B8D52E; background: rgba(184,213,46,0.08); }

        .ca-nav-link:hover { background: rgba(184,213,46,0.08) !important; color: #c8e860 !important; }
        .ca-nav-active:hover { background: #B8D52E !important; color: #0d2416 !important; }
        ...
        @media (min-width: 1024px) {
          .ca-desktop-sidebar { display: block; }
          .ca-mobile-drawer   { display: none !important; }
          .ca-backdrop        { display: none !important; }
          .ca-mobile-close    { display: none !important; }
          .ca-desktop-toggle  { display: flex !important; }
        }

        @media (max-width: 1023px) {
          .ca-desktop-sidebar { display: none !important; }
          .ca-mobile-drawer   { display: block; }
          .ca-backdrop        { display: block; }
          .ca-desktop-toggle  { display: none !important; }
          .ca-mobile-close    { display: flex !important; }
        }
      `}</style>

      <div className="ca-desktop-sidebar" style={{ width: collapsed ? 64 : 240 }}>
        {sidebarContent}
      </div>

      {isOpen && <div className="ca-backdrop" onClick={onClose} aria-hidden="true" />}

      <div
        className="ca-mobile-drawer"
