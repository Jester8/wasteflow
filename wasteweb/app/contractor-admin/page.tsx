...
export default function ContractorAdminOverview() {
  const { contractors, myRequests, loading } = useContractorAdminData();

  const statusRows = useMemo(() => {
    const counts: Record<string, number> = {};
    myRequests.forEach((r) => { counts[r.status] = (counts[r.status] || 0) + 1; });
  ...
  const contractorLeaderboard = useMemo(() => {
    ...
    return contractors
      .map((c) => ({ label: c.fullName, value: counts[c.id] || 0 }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 5);
  }, [contractors, myRequests]);
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: "0.85rem", color: "#6b8f7a", margin: "4px 0 20px" }}>
        Live activity across every contractor you have onboarded.
      </p>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
            ...
            <KpiCard label="Total Requests" value={myRequests.length} />
          ...
          <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
            <BarList title="Top contractors by requests" rows={contractorLeaderboard} emptyLabel="No contractors yet." />
          </div>
        </div>
      )}
    </div>
